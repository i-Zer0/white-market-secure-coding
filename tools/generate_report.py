...
def build_story(styles, github_url):
    ...
    story += [
        styled_table(
            [
                ["위협", "공격 영향", "핵심 대응", "근거"],
                ["SQL Injection", "DB 조회·변조", "모든 값 파라미터 바인딩, 동적 컬럼 허용 목록", "app.py:1905, 2476, 3762"],
                ["XSS", "세션 탈취·임의 요청", "html.escape, textContent escape, strict CSP", "app.py:165, 1577; chat.js"],
                ["CSRF", "사용자·관리 작업 위조", "세션별 토큰, 중앙 POST 검증, SameSite", "app.py:1249, 1445"],
                ["계정 탈취", "계정·거래 제어", "PBKDF2, 잠금, 본인인증, TOTP·복구코드", "app.py:453, 1891"],
                ["권한 상승·IDOR", "타 사용자·관리 데이터 접근", "관리자/당사자/소유자 조건을 쿼리에 결합", "app.py:1464, 4138, 4218"],
                ["악성 이미지", "저장소 고갈·브라우저 공격", "Base64·크기·JPEG 구조 검증, 난수명, Rate Limit", "app.py:213-315"],
                ["백업 유출·변조", "전체 개인정보 노출·훼손", "AES-GCM, integrity_check, 재인증, 세션 폐기", "app.py:335-375, 4658"],
            ],
