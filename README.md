...
- 로그인 보안: 접속 기기와 최근 로그인 기록, 새 환경 로그인 감지, 다른 세션 일괄 종료, 로그인 5회 실패 시 휴대전화 추가 본인인증
...
- 평소 로그인에는 추가 인증 입력을 표시하지 않으며, 실패가 5회 누적된 계정에만 5분 만료·최대 5회 입력 제한의 휴대전화 인증을 자동 적용합니다.
- 조건부 로그인 인증번호는 PBKDF2 해시로 저장하고 한 번 사용하면 재사용할 수 없으며, 발급과 인증 완료 과정을 계정 감사 기록에 남깁니다.
