...
- 유저: 휴대전화 번호를 포함한 회원가입, 아이디 중복확인, GPS 기반 구 단위 동네 입력, 로그인, 본인인증 비밀번호 재설정, 현재 비밀번호 확인 후 비밀번호 변경, JPG·PNG 프로필 사진을 포함한 마이페이지, 사용자 조회, 사용자 차단, 5점 만점 별점
- 소통: SSE 실시간 메시지 수신, 읽음·접속 상태, 채팅 이미지, 상품별 1대1 채팅, 가격 제안과 변경 이력
...
- 거래: 채팅 상대를 판매자가 예약중으로 지정, 채팅방 내 구매자 WM 가상 포인트 송금, 송금 후 판매자 거래 완료, 거래 금액 스냅샷, 안전 체크리스트, 노쇼·사기 의심 신고 연결, 채팅·제안·상태·송금 증빙 이력
...
- 알림·설정: 채팅·가격 인하·거래 상태·공지·보안 유형별 수신 설정, 읽지 않은 개수, 관련 상품·채팅 바로가기, 일괄 읽음 처리, 알림·보안·개인정보 설정 화면 분리
- 로그인 보안: 접속 기기와 최근 로그인 기록, 새 환경 로그인 감지, 다른 세션 일괄 종료, 로그인 5회 실패 시 계정 잠금과 휴대전화 본인인증 비밀번호 재설정
...
- 정지 계정은 로그인할 수 없습니다.
- 존재하는 계정의 로그인 실패만 누적하며, 5회 실패하면 올바른 비밀번호 확인 후 등록된 휴대전화의 일회용 인증번호를 추가로 검증합니다.
...
- WM 포인트는 실제 현금 가치가 없는 교육용 수단이며 가입 시 1,000,000포인트가 지급됩니다. 판매자가 예약중으로 확정한 거래의 구매자만 확정 거래 금액을 한 번 송금할 수 있으며, 차감·적립·원장 기록은 하나의 DB 트랜잭션으로 처리합니다.
- 상품 가격이나 수취인을 클라이언트 입력으로 신뢰하지 않고 거래 생성 시 저장한 금액과 거래 당사자 정보를 서버에서 다시 조회합니다. 잔액 부족·중복 송금·제3자 요청은 원장과 잔액을 변경하지 않고 거부합니다.
...
- 가격 제안과 판매자의 거래 상태 변경은 참여자·상품 소유 관계를 서버에서 검증하고 변경 이력 테이블에 보관합니다.
- 로그인 세션에는 생성·최근 활동·IP·User-Agent를 기록하며, 새 환경 로그인은 보안 알림으로 전달합니다.
- 평소 로그인에는 추가 인증 입력을 표시하지 않으며, 실패가 5회 누적되면 자동으로 계정 잠금 해제 화면으로 전환합니다. 등록 휴대전화 확인과 5분 만료·최대 5회 입력 제한의 인증번호 검증 후 새 비밀번호를 설정해야 합니다.
- 계정 잠금 해제용 비밀번호 재설정 인증번호는 PBKDF2 해시로 저장하며, 한 번 사용한 인증번호는 재사용할 수 없습니다.
...
비밀번호 재설정과 로그인 실패 누적 후 추가 인증에 휴대전화 인증번호를 사용합니다. 로컬 개발 환경에서는 기능 확인을 위해 인증번호가 결과 화면에 표시됩니다. 실제 배포에서는 화면에 인증번호가 노출되지 않도록 `SMS_WEBHOOK_URL`을 설정해야 합니다. 서버는 아래 JSON을 해당 주소에 `POST`합니다.
